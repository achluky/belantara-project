...
class Status_kepegawaian {
	
	private $table = 'refStatusKepegawaian';
	private $header = array('No.','Status Kepegawaian');
	...
	public function run($controller, $change, $id) { 
        ...
        $controller->navigation->setSubMenuActive('status_kepegawaian');
		$controller->navigation->setBreadcrumbReference('status_kepegawaian');
	...
	public function view($controller, $id){
		...
		$controller->navigation->setBreadcrumbReference('status_kepegawaian', 'view');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
                    'data_view'=>  $controller->model_referensi->get_once($this->table, $id),
                    'title'=> $controller->lang->line('navigation.navbar.referensi.status_kepegawaian'),
        ...
        $controller->smartyci->display('admin/reference/status_kepegawaian_view.tpl'); 
	}
	public function add($controller){
	
		$controller->navigation->setBreadcrumbReference('status_kepegawaian', 'add');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
                    'title'=> $controller->lang->line('navigation.navbar.referensi.status_kepegawaian'),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/status_kepegawaian_add.tpl'); 
	
	}
	public function save($controller){
		$data = array(
            'Nama' => $controller->input->post('status_kepegawaian')
        );
		$id = $controller->model_referensi->save($this->table, $data);
		if ($id!=null) {
            $alert = url_title("success");
			redirect('referensi/status_kepegawaian/edit/' . $id. '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('referensi/status_kepegawaian/add?n=' . $alert, 'refresh');
        }
	}
	public function edit($controller, $id=null){
		$controller->navigation->setBreadcrumbReference('status_kepegawaian', 'edit');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/status_kepegawaian_edit.tpl'); 
	}
	public function update($controller){
		...
		if ($controller->model_referensi->update($this->table, $data)) {
            $alert = url_title("success");
			redirect('referensi/status_kepegawaian/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('referensi/status_kepegawaian/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        }
	...
	public function delete($controller, $id){
		...
		redirect('referensi/status_kepegawaian/?n=' . $alert, 'refresh');
	}
	public function export($controller){ 
        ...
        $controller->excel_generate->setFileName("Status_Kepegawaian");
	...
	public function listdata($controller){ 
		 $data = array(
					...
					'label' => array(
							...
							'tabel_status_kepegawaian' => $controller->lang->line('label.status_kepegawaian'),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/status_kepegawaian.tpl'); 
	}
	public function listdata_json($controller){ 
        ...
        foreach ($list as $item) {
            ...
            //add html for action
            $row[] = '<a href="' . base_url('referensi/status_kepegawaian') . '/view/' . $item->ID . '"  class="btn btn-xs"><i class="fa fa-file fa-fw"></i> '.$controller->lang->line('label.tabel.view').'</a>';
            $data[] = $row;
        }
