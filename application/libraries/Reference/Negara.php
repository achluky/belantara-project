...
class Negara {
	...
	private $table = 'refNegara';
	private $header = array('No.','Kode','Negara');
	...
	public function run($controller, $change, $id) { 
        ...
        $controller->navigation->setSubMenuActive('negara');
		$controller->navigation->setBreadcrumbReference('negara');
	...
	public function view($controller, $id){
		$controller->navigation->setBreadcrumbReference('negara', 'view');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
                    'data_view'=>  $controller->model_referensi->get_once($this->table, $id),
                    'title'=> $controller->lang->line('navigation.navbar.referensi.negara'),
        ...
        $controller->smartyci->display('admin/reference/negara_view.tpl'); 
	}
	public function add($controller){
		$controller->navigation->setBreadcrumbReference('negara', 'add');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
                    'title'=> $controller->lang->line('navigation.navbar.referensi.negara'),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/negara_add.tpl'); 
	
	}
	public function save($controller){
		$data = array(
            'Kode' => $controller->input->post('code'),
            'Nama' => $controller->input->post('negara')
        );
		$id = $controller->model_referensi->save($this->table, $data);
		if ($id!=null) {
            $alert = url_title("success");
			redirect('referensi/negara/edit/' . $id. '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('referensi/negara/add?n=' . $alert, 'refresh');
        }
	}
	public function edit($controller, $id=null){
		$controller->navigation->setBreadcrumbReference('negara', 'edit');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/negara_edit.tpl'); 
	}
	public function update($controller){
		...
		if ($controller->model_referensi->update($this->table, $data)) {
            $alert = url_title("success");
			redirect('referensi/negara/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('referensi/negara/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        }
	...
	public function delete($controller, $id){
		...
		redirect('referensi/negara/?n=' . $alert, 'refresh');
	}
	public function export($controller){ 
        ...
        $controller->excel_generate->setFileName("Negara");
	...
	public function listdata($controller){ 
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/negara.tpl'); 
	}
	public function listdata_json($controller){ 
        ...
        foreach ($list as $item) {
            ...
            //add html for action
            $row[] = '<a href="' . base_url('referensi/negara') . '/view/' . $item->ID . '"  class="btn btn-xs"><i class="fa fa-file fa-fw"></i> '.$controller->lang->line('label.tabel.view').'</a>';
            $data[] = $row;
        }
