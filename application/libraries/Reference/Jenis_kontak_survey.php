...
class Jenis_kontak_survey {
	
	private $table = 'refJenisKontakSurvey';
	private $header = array('No.','Jenis Kontak Survey');
	...
	public function run($controller, $change, $id) { 
        ...
        $controller->navigation->setSubMenuActive('jenis_kontak_survey');
		$controller->navigation->setBreadcrumbReference('jenis_kontak_survey');
	...
	public function view($controller, $id){
		...
		$controller->navigation->setBreadcrumbReference('jenis_kontak_survey', 'view');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
                    'data_view'=>  $controller->model_referensi->get_once($this->table, $id),
                    'title'=> $controller->lang->line('navigation.navbar.referensi.jenis_kontak_survey'),
        ...
        $controller->smartyci->display('admin/reference/jenis_kontak_survey_view.tpl'); 
	
	}
	public function add($controller){
	
		$controller->navigation->setBreadcrumbReference('jenis_kontak_survey', 'add');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
                    'title'=> $controller->lang->line('navigation.navbar.referensi.jenis_kontak_survey'),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/jenis_kontak_survey_add.tpl'); 
	
	}
	public function save($controller){
		$data = array(
            'Nama' => $controller->input->post('jenis_kontak_survey')
        );
		$id = $controller->model_referensi->save($this->table, $data);
		if ($id!=null) {
            $alert = url_title("success");
			redirect('referensi/jenis_kontak_survey/edit/' . $id. '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('referensi/jenis_kontak_survey/add?n=' . $alert, 'refresh');
        }
	}
	public function edit($controller, $id=null){
		$controller->navigation->setBreadcrumbReference('jenis_kontak_survey', 'edit');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/jenis_kontak_survey_edit.tpl'); 
	}
	public function update($controller){
		...
		if ($controller->model_referensi->update($this->table, $data)) {
            $alert = url_title("success");
			redirect('referensi/jenis_kontak_survey/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('referensi/jenis_kontak_survey/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        }
	...
	public function delete($controller, $id){
		...
		redirect('referensi/jenis_kontak_survey/?n=' . $alert, 'refresh');
	}
	public function export($controller){ 
        ...
        $controller->excel_generate->setFileName("Jenis_Kontak_Survey");
	...
	public function listdata($controller){ 
		 $data = array(
					...
					'label' => array(
							...
							'tabel_jenis_kontak_survey' => $controller->lang->line('label.jenis_kontak_survey'),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/jenis_kontak_survey.tpl'); 
	}
	public function listdata_json($controller){ 
        ...
        foreach ($list as $item) {
            ...
            //add html for action
            $row[] = '<a href="' . base_url('referensi/jenis_kontak_survey') . '/view/' . $item->ID . '"  class="btn btn-xs"><i class="fa fa-file fa-fw"></i> '.$controller->lang->line('label.tabel.view').'</a>';
            $data[] = $row;
        }
