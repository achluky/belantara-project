...
class Prestasi_mahasiswa {
	
	private $table = 'refPrestasiMahasiswa';
	private $header = array('No.','Prestasi Mahasiswa');
	...
	public function run($controller, $change, $id) { 
        ...
        $controller->navigation->setSubMenuActive('prestasi_mahasiswa');
		$controller->navigation->setBreadcrumbReference('prestasi_mahasiswa');
	...
	public function view($controller, $id){
		$controller->navigation->setBreadcrumbReference('prestasi_mahasiswa', 'view');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
                    'data_view'=>  $controller->model_referensi->get_once($this->table, $id),
                    'title'=> $controller->lang->line('navigation.navbar.referensi.prestasi_mahasiswa'),
        ...
        $controller->smartyci->display('admin/reference/prestasi_mahasiswa_view.tpl'); 
	}
	public function add($controller){
	
		$controller->navigation->setBreadcrumbReference('prestasi_mahasiswa', 'add');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
                    'title'=> $controller->lang->line('navigation.navbar.referensi.prestasi_mahasiswa'),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/prestasi_mahasiswa_add.tpl'); 
	
	}
	public function save($controller){
		$data = array(
            'Nama' => $controller->input->post('prestasi_mahasiswa')
        );
		$id = $controller->model_referensi->save($this->table, $data);
		if ($id!=null) {
            $alert = url_title("success");
			redirect('referensi/prestasi_mahasiswa/edit/' . $id. '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('referensi/prestasi_mahasiswa/add?n=' . $alert, 'refresh');
        }
	}
	public function edit($controller, $id=null){
		$controller->navigation->setBreadcrumbReference('prestasi_mahasiswa', 'edit');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/prestasi_mahasiswa_edit.tpl'); 
	}
	public function update($controller){
		...
		if ($controller->model_referensi->update($this->table, $data)) {
            $alert = url_title("success");
			redirect('referensi/prestasi_mahasiswa/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('referensi/prestasi_mahasiswa/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        }
	...
	public function delete($controller, $id){
		...
		redirect('referensi/prestasi_mahasiswa/?n=' . $alert, 'refresh');
	}
	public function export($controller){ 
        ...
        $controller->excel_generate->setFileName("Prestasi_Mahasiswa");
	...
	public function listdata($controller){ 
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/reference/prestasi_mahasiswa.tpl'); 
	}
	public function listdata_json($controller){ 
        ...
        foreach ($list as $item) {
            ...
            //add html for action
            $row[] = '<a href="' . base_url('referensi/prestasi_mahasiswa') . '/view/' . $item->ID . '"  class="btn btn-xs"><i class="fa fa-file fa-fw"></i> '.$controller->lang->line('label.tabel.view').'</a>';
            $data[] = $row;
        }
