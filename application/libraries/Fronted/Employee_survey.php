...
class Employee_survey {
    ...
    public function view(){
        $this->controller->load->model("model_wcu_employee_reputation_employee_survey");
        $data = array(
                    'url'=> current_url(),
        ...
        $this->controller->smartyci->display('frontend/view/employee_survey.tpl');
    }
    public function list_(){
        $this->controller->load->model("model_wcu_employee_reputation_employee_survey");
        $list = $this->controller->model_wcu_employee_reputation_employee_survey->get_datatables($this->table, $this->column, $this->order);
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->controller->model_wcu_employee_reputation_employee_survey->count_all($this->table),
            "recordsFiltered" => $this->controller->model_wcu_employee_reputation_employee_survey->count_filtered($this->table, $this->column, $this->order),
            "data" => $data
        );
