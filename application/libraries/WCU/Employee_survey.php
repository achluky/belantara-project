...
class Employee_survey {
	...
	private $header_column = array('AUTONUMBER','JenisKontakSurvey', 'NamaDepan','NamaBelakang','Gelar','Email','Telepon','Negara','Universitas','Pekerjaan','PosisiPekerjaan', 'SektorPekerjaan', 'Departemen', 'Institusi');
	...
	public function run($controller, $change, $id=NULL) { 
		$controller->load->model('model_wcu_employee_reputation_employee_survey');
		$controller->navigation->setMenuActive('wur');
        $controller->navigation->setSubMenuActive('employee_reputation');
		$controller->navigation->setBreadcrumbWurEmployeeReputation('employee_survey');
	...
	public function view($controller, $id=null){
		...
		$controller->navigation->setBreadcrumbWurEmployeeReputation('employee_survey', 'view');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$data = array(
                    'url'=> current_url(),
                    'alert' => $controller->input->get('n'),
                    'data_view'=>  $controller->model_wcu_employee_reputation_employee_survey->get_once_with_relation($this->table, $id),
                    'title'=> $controller->lang->line('navigation.navbar.wcu.employee_reputation.wur19'),
                    'last_login' => $sess['last_login'],
					'session' => $sess['username']
        );
		
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/Wcu-employee_reputation/employee_survey_view.tpl'); 
	}
	public function add($controller){
        $sess = $controller->session->userdata('logged_in');
		$controller->navigation->setBreadcrumbWurEmployeeReputation('employee_survey', 'add');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$negara = $controller->model_wcu_employee_reputation_employee_survey->get_all_negara();
		$jenisKontakSurvey = $controller->model_wcu_employee_reputation_employee_survey->get_all_jenis_kontak_survey();
        ...
        $controller->smartyci->display('admin/Wcu-employee_reputation/employee_survey_add.tpl');
	}
	public function save($controller){
		...
		$id = $controller->model_wcu_employee_reputation_employee_survey->save($this->table, $data);
		if ($id!=null) {
            $alert = url_title("success");
			redirect('wcu/employee/employee_survey/edit/' . $id. '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('wcu/employee/employee_survey/add?n=' . $alert, 'refresh');
        }
	}
	public function edit($controller, $id=null){
        $sess = $controller->session->userdata('logged_in');
		$controller->navigation->setBreadcrumbWurEmployeeReputation('employee_survey', 'edit');
		$controller->smartyci->assign('breadcrumb',$controller->navigation->getBreadcrumb());
		$negara = $controller->model_wcu_employee_reputation_employee_survey->get_all_negara();
		$jenisKontakSurvey = $controller->model_wcu_employee_reputation_employee_survey->get_all_jenis_kontak_survey();
		...
		$data = array(
                    'url'=> current_url(),
                    'alert' => $controller->input->get('n'),
                    'data_view'=>  $controller->model_wcu_employee_reputation_employee_survey->get_once($this->table, $id),
					'title'=> $controller->lang->line('navigation.navbar.wcu.employee_reputation.wur19'),
                    'last_login' => $sess['last_login'],
					'session' => $sess['username']
        );
        ...
        $controller->smartyci->assign('negara',$negara);
        $controller->smartyci->assign('jenisKontakSurvey',$jenisKontakSurvey);
        $controller->smartyci->display('admin/Wcu-employee_reputation/employee_survey_edit.tpl');
	}
	public function update($controller){
		...
		if ($controller->model_wcu_employee_reputation_employee_survey->update($this->table, $data)) {
            $alert = url_title("success");
			redirect('wcu/employee/employee_survey/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        } else {
            $alert = url_title("failed");
			redirect('wcu/employee/employee_survey/edit/' . $controller->input->post('id'). '?n=' . $alert, 'refresh');
        }
	...
	public function delete($controller, $id=null){
		if($controller->model_wcu_employee_reputation_employee_survey->delete($this->table, $id)) {
            $alert = url_title("success");
        } else {
            $alert = url_title("failed");
        }
		redirect('wcu/employee/employee_survey/?n=' . $alert, 'refresh');
	}
	public function export($controller){
		$controller->load->library('excel_generate');
        $data = $controller->model_wcu_employee_reputation_employee_survey->get_exsport($this->table, $this->header, $this->header_column);
        $controller->excel_generate->setFileName("Employee_Survey");
	...
	public function listdata($controller){ 
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/Wcu-employee_reputation/employee_survey.tpl'); 
	}
	
	public function listdata_json($controller){ 
		$list = $controller->model_wcu_employee_reputation_employee_survey->get_datatables($this->table, $this->column, $this->order);
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $item) {
            ...
            $row[] = $item->Departemen;
            $row[] = $item->Institusi;
		   $row[] = '<a href="' . base_url('wcu/employee/employee_survey') . '/view/' . $item->ID . '"  class="btn btn-xs"><i class="fa fa-file fa-fw"></i> '.$controller->lang->line('label.tabel.view').'</a>
                    <a href="' . base_url('wcu/employee/employee_survey') . '/edit/' . $item->ID . '"  class="btn btn-xs"><i class="fa fa-edit fa-fw"></i> '.$controller->lang->line('label.tabel.edit').'</a>
                    <a href="#delete" class="btn btn-xs btn-danger" onClick="delete_data(\'' . base_url('wcu/employee/employee_survey') . '/delete/' . $item->ID. '\')"><i class="fa fa-remove fa-fw"></i> '.$controller->lang->line('label.tabel.delete').'</a>
					';
		    $data[] = $row;
        }
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $controller->model_wcu_employee_reputation_employee_survey->count_all($this->table),
            "recordsFiltered" => $controller->model_wcu_employee_reputation_employee_survey->count_filtered($this->table, $this->column, $this->order),
            "data" => $data,
        );
    ...
    public function sematkan($controller){
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/Wcu-employee_reputation/employee_survey_sematkan.tpl'); 
    }
}
