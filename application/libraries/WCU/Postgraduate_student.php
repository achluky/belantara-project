...
class Postgraduate_student {
	...
	public function run($controller, $change, $id=NULL) { 
		$controller->load->model('model_wcu_data_university_postgraduate_student');
        $controller->navigation->setMenuActive('wur');
        $controller->navigation->setSubMenuActive('data_university');
        $controller->navigation->setBreadcrumbWurDataUniversity('postgraduate_student',$change);
	...
	public function listdata($controller){ 
		...
		$data = array(
                    ...
                    'data_departement'=>$controller->model_wcu_data_university_postgraduate_student->getListDepartement(),
                    'title'=> 'Graduate/Postgraduate Student',
                    'last_login' => $sess['last_login'],
					'session' => $sess['username']
        );
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/Wcu-data-university/wcu-data-university-postgraduate_student.tpl'); 
	}
	
	public function view($controller){
		$nim 	= $controller->input->get("nim");
		$data 	= $controller->model_wcu_data_university_postgraduate_student->getStudenOne($nim);
		$row 	= $data->row();
		$arr = array(
				'nim' => $nim, 
				'Nama'=> $row->Nama,
	...
	public function export($controller){ 
        ...
        $data = $controller->model_wcu_data_university_postgraduate_student->get_exsport_graduate_mahasiswa($controller->input->get('d'),$controller->input->get('s'));
	...
	public function departement($controller,$change){
		...
		$data = array(
		            ...
		            'departemen_title'=> $controller->model_wcu_data_university_postgraduate_student->getMayor($controller->input->get('d')),
		            'departemen'=> $d[1],
		            'strata'=> $d[0],
                    'last_login' => $sess['last_login'],
					'session' => $sess['username']
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/Wcu-data-university/wcu-data-university-postgraduate_student-list.tpl'); 
	}
	...
	public function studentlist($controller){
        ...
        $list = $controller->model_wcu_data_university_postgraduate_student->get_datatables_listStudent($departemen,$strata);
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $controller->model_wcu_data_university_postgraduate_student->count_allStudent($departemen,$strata),
            "recordsFiltered" => $controller->model_wcu_data_university_postgraduate_student->count_filteredStudent($departemen,$strata),
            "data" => $data
        );
	...
	public function sematkan($controller){	
        ...
        $controller->smartyci->assign('data',$data);
        $controller->smartyci->display('admin/Wcu-data-university/wcu-data-university-postgraduate_student-sematkan.tpl'); 
	}
}
