...
class Model_wcu_employee_reputation_employee_survey extends CI_Model {
    ...
    {
          $this->db
                    ->select('t1.ID, t3.Nama AS JenisKontakSurvey, t1.NamaDepan, t1.NamaBelakang, 
                        t1.Gelar, t2.Nama AS Negara,  t1.Universitas, t1.Pekerjaan, t1.Departemen, t1.Institusi')
                    ->from($table.' t1 ')
                    ->join("refNegara  t2","t2.ID = t1.NegaraID", "left")
                    ->join("refJenisKontakSurvey  t3","t3.ID = t1.JenisKontakSurveyID", "left")
					->where('t3.ID', 2);
