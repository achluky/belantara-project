...
class Model_wcu_internasionalization_postgraduate_outbound extends CI_Model {
	...
	public function get_once_with_relation($table, $id, $idMayor)
    {
       $this->db
                    ->select('t1.ID, 
                        t3.NIM, 
                    ...
                    ->where("t7.ID",$idMayor)
                    ->where('t1.ID', $id)
                    ->where("t7.StrataID","3")->or_where("t7.StrataID","4");
		$result	 = $this->db->get()->result_array();
        return (isset($result[0])?$result[0]:array("ID"=>null));
    }
    public function _get_datatables_query($table, $c, $o, $id=NULL)
    {
        $this->db
                    ->select('t1.ID, 
                                t3.NIM, 
        ...
        // TIMESTAMPDIFF(MONTH, t2.TanggalMulai, t2.TanggalSelesai) AS Cmonth'
		#Create main query
		$this->db->select('*');
